...
export default function Dashboard() {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <aside className={`fixed inset-y-0 left-0 z-30 flex w-[220px] flex-col border-r border-border bg-card px-5 py-6 transition-transform lg:translate-x-0 ${menuOpen ? "translate-x-0" : "-translate-x-full"}`}>
      <div className="flex items-center justify-between px-1"><Logo /><button className="text-muted-foreground lg:hidden" onClick={() => setMenuOpen(false)}><X size={20} /></button></div>
      <div className="mt-12"><p className="mb-3 px-3 text-[10px] font-extrabold uppercase tracking-[0.18em] text-muted-foreground">Workspace</p><nav className="space-y-1">{navItems.map(({ label, icon: Icon }, index) => <Link key={label} to={index === 0 ? "/" : `/${label.toLowerCase().replace(/ /g, "-")}`} className={`nav-item ${index === 0 ? "active" : ""}`}><Icon size={17} />{label}{label === "Transactions" && <span className="ml-auto rounded-full bg-[#edf7f3] px-2 py-0.5 text-[10px] text-[#39856e] dark:bg-primary/20 dark:text-primary">24</span>}</Link>)}</nav></div>
      <div className="mt-9"><p className="mb-3 px-3 text-[10px] font-extrabold uppercase tracking-[0.18em] text-muted-foreground">Manage</p><nav className="space-y-1"><Link to="/investments" className="nav-item"><TrendingUp size={17} />Investments</Link><Link to="/financial-health" className="nav-item"><ShieldCheck size={17} />Financial health</Link><Link to="/settings" className="nav-item"><Settings size={17} />Settings</Link></nav></div>
      <div className="mt-auto rounded-2xl bg-secondary p-4"><div className="mb-3 flex h-8 w-8 items-center justify-center rounded-lg bg-primary/20 text-primary"><Sparkles size={16} /></div><p className="text-[12px] font-bold text-foreground">Your AI coach is ready</p><p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">Get smarter suggestions based on your spending.</p><button className="mt-3 text-[11px] font-bold text-[#1f856b] dark:text-primary">View insights <ChevronRight className="ml-1 inline" size={13} /></button></div>
    </aside>
    ...
    <main className="lg:pl-[220px]">
      <header className="relative z-50 flex h-[76px] items-center justify-between border-b border-border bg-background/95 px-5 backdrop-blur-md sm:px-8"><div className="flex items-center gap-4"><button className="text-muted-foreground lg:hidden" onClick={() => setMenuOpen(true)}><Menu size={22} /></button><div className="relative hidden sm:block"><Search className="absolute left-3 top-2.5 text-muted-foreground" size={16} /><input className="h-9 w-56 rounded-xl border-0 bg-secondary pl-9 text-xs outline-none placeholder:text-muted-foreground" placeholder="Search anything..." /></div><div className="hidden items-center gap-2 rounded-xl bg-secondary px-3 py-2 text-[11px] font-semibold text-muted-foreground md:flex"><span className="h-1.5 w-1.5 rounded-full bg-[#36b78e]" />All systems normal</div></div><div className="flex items-center gap-4"><button className="relative text-muted-foreground"><Bell size={19} /><span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full border-2 border-white bg-[#f17767]" /></button><div className="h-6 w-px bg-border" /><div ref={profileRef} className="relative"><button onClick={() => setProfileOpen(!profileOpen)} className="group flex items-center gap-2.5 rounded-xl p-1.5 transition hover:bg-secondary/50" aria-expanded={profileOpen}><div className="relative flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-xs font-extrabold text-primary ring-2 ring-transparent transition group-hover:ring-primary/20">{user?.fullName?.charAt(0) || "U"}<span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-white bg-[#43b88e]" /></div><div className="hidden text-left sm:block"><p className="text-[11px] font-bold text-foreground">{user?.fullName || "User"}</p><p className="text-[10px] text-muted-foreground">Personal account</p></div><ChevronDown size={14} className={`text-muted-foreground transition-transform ${profileOpen ? "rotate-180" : ""}`} /></button>{profileOpen && <div className="profile-menu absolute right-0 top-[calc(100%+12px)] z-[100] w-[calc(100vw-2rem)] max-w-64 overflow-hidden rounded-2xl border border-border bg-card p-2 shadow-2xl"><div className="rounded-xl bg-secondary p-3"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/20 text-sm font-extrabold text-primary">{user?.fullName?.charAt(0) || "U"}</div><div><p className="text-xs font-extrabold text-foreground">{user?.fullName || "User"}</p><p className="mt-0.5 text-[10px] text-muted-foreground">{user?.email || ""}</p></div></div><div className="mt-3 flex items-center gap-1.5 text-[10px] font-bold text-primary"><span className="h-1.5 w-1.5 rounded-full bg-[#43b88e]" />Personal account</div></div><div className="my-2 h-px bg-border" /><Link to="/profile-settings" onClick={() => setProfileOpen(false)} className="profile-menu-item"><UserRound size={15} />Profile settings</Link><Link to="/privacy-security" onClick={() => setProfileOpen(false)} className="profile-menu-item"><ShieldCheck size={15} />Privacy & security</Link><button onClick={() => { logout(); navigate("/"); }} className="profile-menu-item text-destructive hover:bg-destructive/10 w-full text-left"><LogOut size={15} />Sign out</button></div>}</div></div></header>
      <motion.div key="dashboard" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, ease: "easeOut" }} className="dashboard-content mx-auto max-w-[1450px] px-5 py-7 sm:px-8 lg:px-10"><div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-[11px] font-bold uppercase tracking-[0.17em] text-[#1f9b78]">{formattedDate}</p><h1 className="text-[30px] font-extrabold tracking-[-0.05em] text-foreground sm:text-[34px]">{greeting}, {user?.fullName?.split(' ')[0] || "User"} <span className="text-[#1f9b78]">✦</span></h1><p className="mt-1 text-[13px] text-muted-foreground">Here’s your financial pulse for this month.</p></div><div className="flex gap-2"><button onClick={() => setShowAdd(true)} className="flex items-center gap-2 rounded-xl bg-[#1f7a63] px-4 py-2.5 text-xs font-bold text-white shadow-lg shadow-emerald-900/10 transition hover:-translate-y-0.5 hover:bg-[#196b56]"><Plus size={16} />Add transaction</button><button className="flex items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2.5 text-xs font-bold text-foreground"><Upload size={15} />Upload slip</button></div></div>
