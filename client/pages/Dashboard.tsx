...
export default function Dashboard() {
  ...
  const { transactions, goals, cashFlow, financialHealthScore, expectedSalary } = useAppData();
  ...
  const latestSalary = expectedSalary;
  ...
  return (
    ...
    <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="mb-2 text-[11px] font-bold uppercase tracking-[0.17em] text-[#1f9b78]">{formattedDate}</p><h1 className="text-[30px] font-extrabold tracking-[-0.05em] text-foreground sm:text-[34px]">{greeting}, {user?.fullName?.split(' ')[0] || "User"} <span className="text-[#1f9b78]">✦</span></h1><p className="mt-1 text-[13px] text-muted-foreground">Here’s your financial pulse for this month.</p></div><div className="flex gap-2"><button onClick={() => setShowAdd(true)} className="flex items-center gap-2 rounded-xl bg-[#1f7a63] px-4 py-2.5 text-xs font-bold text-white shadow-lg shadow-emerald-900/10 transition hover:-translate-y-0.5 hover:bg-[#196b56]"><Plus size={16} />Add transaction</button><button className="flex items-center gap-2 rounded-xl border border-border bg-card px-3.5 py-2.5 text-xs font-bold text-foreground"><Upload size={15} />Upload slip</button></div></div>
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatCard title="Total balance" value={currentBalance} prefix="₹" detail="Updated just now" icon={Wallet} tone="tone-green" /><StatCard title="Latest Salary" value={latestSalary} prefix="₹" detail="Most recent credit" icon={ArrowDownRight} tone="tone-blue" /><StatCard title="Monthly income" value={totalIncome} prefix="₹" detail="Updated just now" icon={ArrowDownRight} tone="tone-blue" /><StatCard title="Monthly expenses" value={totalExpense} prefix="₹" detail="Updated just now" icon={ArrowUpRight} tone="tone-peach" /><StatCard title="Savings rate" value={savingsRate} suffix="%" detail={savingsRate >= 40 ? "Above your 40% target" : "Below your 40% target"} icon={Target} tone="tone-lilac" progress={Math.min(100, savingsRate)} /></div>
        <div className="mt-5 grid gap-5 xl:grid-cols-[1.6fr_1fr]"><section className="overflow-hidden rounded-2xl border border-[#e6efeb] dark:border-border bg-white dark:bg-card p-5 shadow-[0_5px_25px_rgba(32,85,68,0.03)] dark:shadow-none sm:p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-[15px] font-extrabold">Income vs expenses</h2><p className="mt-1 text-[11px] text-[#91a59f] dark:text-muted-foreground">Your cash flow over the last 7 months</p></div><select value={period} onChange={e => setPeriod(e.target.value)} className="rounded-lg border border-[#e6efeb] dark:border-border bg-[#fbfdfc] dark:bg-secondary px-2.5 py-2 text-[11px] font-bold text-[#628078] dark:text-foreground outline-none"><option>This year</option><option>Last year</option></select></div><div className="mb-3 flex gap-5 text-[11px] font-semibold text-[#7f9790] dark:text-muted-foreground"><span><i className="mr-1.5 inline-block h-2 w-2 rounded-full bg-[#259777]" />Income</span><span><i className="mr-1.5 inline-block h-2 w-2 rounded-full bg-[#f29b82]" />Expenses</span></div><ResponsiveContainer width="100%" height={235}><AreaChart data={cashFlow} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}><defs><linearGradient id="incomeFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#55bd9b" stopOpacity={0.24} /><stop offset="100%" stopColor="#55bd9b" stopOpacity={0} /></linearGradient><linearGradient id="expenseFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#f29b82" stopOpacity={0.19} /><stop offset="100%" stopColor="#f29b82" stopOpacity={0} /></linearGradient></defs><CartesianGrid vertical={false} stroke="#eef4f1" /><XAxis dataKey="m" axisLine={false} tickLine={false} tick={{ fill: "#9badA7", fontSize: 10 }} dy={8} /><YAxis axisLine={false} tickLine={false} tick={{ fill: "#9badA7", fontSize: 10 }} tickFormatter={(v) => `₹${(v/1000).toFixed(0)}k`} /><Tooltip contentStyle={{ border: "0", borderRadius: "10px", fontSize: "11px", boxShadow: "0 6px 20px #235e4e18" }} /><Area type="monotone" dataKey="income" stroke="#299a7b" strokeWidth={2.5} fill="url(#incomeFill)" /><Area type="monotone" dataKey="expense" stroke="#ef9b82" strokeWidth={2.5} fill="url(#expenseFill)" /></AreaChart></ResponsiveContainer></section><section className="overflow-hidden rounded-2xl border border-[#e6efeb] dark:border-border bg-white dark:bg-card p-5 shadow-[0_5px_25px_rgba(32,85,68,0.03)] dark:shadow-none sm:p-6"><div className="flex items-start justify-between"><div><h2 className="text-[15px] font-extrabold">Financial health</h2><p className="mt-1 text-[11px] text-[#91a59f] dark:text-muted-foreground">Your overall money wellness</p></div><button className="text-[#a2b4ae] dark:text-muted-foreground hover:text-foreground"><MoreHorizontal size={18} /></button></div><div className="health-gauge relative mx-auto mt-7 h-[145px] w-[250px] overflow-hidden flex justify-center"><svg width="250" height="145" viewBox="0 0 250 145" className="overflow-visible"><path d="M 18,125 A 107,107 0 0,1 232,125" fill="none" stroke="currentColor" strokeWidth="18" strokeLinecap="round" className="text-[#edf3f0] dark:text-secondary" /><path d="M 18,125 A 107,107 0 0,1 232,125" fill="none" stroke="#42b38e" strokeWidth="18" strokeLinecap="round" strokeDasharray="336" strokeDashoffset={336 - (336 * financialHealthScore / 100)} className="transition-all duration-1000 ease-out" /></svg><div className="absolute inset-x-0 top-[65px] text-center"><p className="text-[40px] font-extrabold tracking-[-0.07em] text-[#194f40] dark:text-foreground"><CountUp end={financialHealthScore} duration={2} /></p><p className="text-[11px] font-bold text-[#65a18c] dark:text-[#42b38e]">{financialHealthScore >= 80 ? "Excellent" : financialHealthScore >= 50 ? "Good" : "Needs Attention"}</p></div></div><div className="mt-4 flex items-center justify-between rounded-xl bg-[#f6faf8] dark:bg-secondary px-3 py-2.5"><span className="text-[11px] font-semibold text-[#78918a] dark:text-muted-foreground">Top {Math.max(1, 100 - financialHealthScore)}% of users</span><span className="flex items-center gap-1 text-[11px] font-bold text-[#299a7b]"><ArrowUpRight size={13} />Dynamic</span></div><p className="mt-4 text-[11px] leading-relaxed text-[#78918a] dark:text-muted-foreground">{financialHealthScore >= 80 ? "You're doing great! Keep your savings rate high to reach goals sooner." : "Consider reviewing your expenses to improve your financial health score."}</p></section></div>
